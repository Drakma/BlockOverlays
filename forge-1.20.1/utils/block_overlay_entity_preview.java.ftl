...
private static void blockOverlayWarnNotSpawnEgg(Item item) {
	if (BLOCK_OVERLAY_ENTITY_PREVIEW_WARNED_ITEMS.add(item))
		BLOCK_OVERLAY_ENTITY_PREVIEW_LOG.warn("BlockOverlays: item '{}' passed to entity overlay is not a spawn egg, nothing will render", net.minecraftforge.registries.ForgeRegistries.ITEMS.getKey(item));
}

private static net.minecraft.world.entity.Entity blockOverlayGetPreviewEntity(net.minecraft.world.entity.EntityType<?> type, net.minecraft.world.level.Level level) {
	...
	try {
		...
		BLOCK_OVERLAY_ENTITY_PREVIEW_LOG.error("BlockOverlays: exception creating preview entity for type '{}'", net.minecraftforge.registries.ForgeRegistries.ENTITY_TYPES.getKey(type), exception);
		BLOCK_OVERLAY_ENTITY_PREVIEW_FAILED.add(type);
		return null;
	}
	if (entity == null) {
		BLOCK_OVERLAY_ENTITY_PREVIEW_LOG.warn("BlockOverlays: could not create preview entity for type '{}'", net.minecraftforge.registries.ForgeRegistries.ENTITY_TYPES.getKey(type));
		BLOCK_OVERLAY_ENTITY_PREVIEW_FAILED.add(type);
		return null;
	}
	BLOCK_OVERLAY_ENTITY_PREVIEW_LOG.info("BlockOverlays: created preview entity for type '{}' ({})", net.minecraftforge.registries.ForgeRegistries.ENTITY_TYPES.getKey(type), entity.getClass().getName());
