...
if (event instanceof SubmitCustomGeometryEvent _overlayEvent) {
	...
	boolean _flipWinding = _side == Direction.UP || _side == Direction.DOWN;
	_overlayEvent.getSubmitNodeCollector().submitCustomGeometry(_poseStack, blockOverlayTextureRenderType(net.minecraft.client.renderer.texture.TextureAtlas.LOCATION_BLOCKS), (_pose, _consumer) -> {
		if (_flipWinding) {
			_consumer.addVertex(_pose, -0.5f, -0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU1(), _finalSprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
			_consumer.addVertex(_pose, -0.5f, 0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU1(), _finalSprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
			_consumer.addVertex(_pose, 0.5f, 0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU0(), _finalSprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
			_consumer.addVertex(_pose, 0.5f, -0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU0(), _finalSprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
		} else {
			_consumer.addVertex(_pose, 0.5f, -0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU0(), _finalSprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
			_consumer.addVertex(_pose, 0.5f, 0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU0(), _finalSprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
			_consumer.addVertex(_pose, -0.5f, 0.5f, 0).setColor(_r, _g, _b, _a).setUv(_finalSprite.getU1(), _finalSprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(0xF000F0).setNormal(_pose, 0, 0, -1);
